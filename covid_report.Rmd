...
```{r}
data_a <- na.omit(covid_data)
annual_percentage_change <- data_a %>% filter(units == "Annual % change") 
mean_annual_percentage_change <- mean(annual_percentage_change$value)
index <- data_a %>% filter(units == "Index") 
mean_index <- mean(index$value)
NA_data <- subset(covid_data,is.na(value))
...
data_frame <- rbind(data_a, data_b)
data_frame$date_last_updated <- as.Date(data_frame$date_last_updated, format = "%d/%m/%y")
```
